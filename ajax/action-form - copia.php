...
if(isset($_REQUEST['forgotemail']) and $_REQUEST['forgotemail']!=""){
	extract($_REQUEST);
	$getUsers	=	$db->getAllRecords('administrador','*',' AND ((useremail="'.$forgotemail.'") OR (numero_empleado="'.$forgotemail.'")) ');
	if(isset($getUsers[0]['id']) and $getUsers[0]['id']!=""){
		
		$token		=	md5(uniqid('lcw-'.mt_rand(),true));
		$update		=	$db->update('administrador',array('token'=>$token),array('id'=>$getUsers[0]['id']));
		...
		*/
		...
		$body	=	'<table bgcolor="#FFFFFF" width="560" style="border:1px solid #ccc; opacity:0.8; font-family:Arial; font-size:14px; line-height:18px;" cellspacing="0" cellpadding="5" border="0" align="center">
						<tbody>
							<tr>
								<td align="center"><strong style="color:#55BDE8; font-size:3em; font-weight:bolder; text-align:center; margin:0px;">CONTRASEÑA ACTUALIZA</strong><br /><br /></td>
							</tr>
							<tr>
								<td><strong>Request</strong></td>
							</tr>
							<tr>
								<td style="color:#4da6e1; font-size: 25px; padding-bottom: 10px; border-bottom: 1px solid #000;">Dear '.isset($getUsers[0]['numero_empleado'])?$getUsers[0]['numero_empleado']:'numero_empleado'.',</td>
							</tr>
							<tr>
							...
							<tr>
								<td width="100%" valign="middle" align="center" style="text-align: center; width: 100%; padding: 5px; color:#FFF; height:60px; background:linear-gradient(to bottom, #33BDBE 0, #06184d 100%);">
									...
									<a style="color: #fff;text-decoration:none;" href="" target="_blank">Cumplimiento de licencias</a>
									</p>
								</td>
							</tr>
						</tbody>
...
if(isset($_REQUEST['signinname']) and $_REQUEST['signinname']!=""){
	extract($_REQUEST);
	$getUsers	=	$db->getAllRecords('administrador','id,numero_empleado,username,useremail,userpassword',' AND ((useremail="'.$signinname.'") OR (numero_empleado="'.$signinname.'")) AND userstatus=1 ');
	if(isset($getUsers[0]['userpassword']) and $getUsers[0]['userpassword']!=""){
		...
		*/
		...
		if(password_verify($signinpassword, $hash)){
			...
			$_SESSION['numero_empleado']		=	$getUsers[0]['numero_empleado'];
			$_SESSION['username']		=	$getUsers[0]['username'];
			echo '<div class="alert alert-success p-1 mt-1"><i class="fa fa-fw fa-thumbs-up"></i> Inicia sesión correctamente <strong>Por favor espera..!</strong></div>|***|1|***|Administrador/PrincipalAdmin/PrincipalAdmin.php';
			exit;
		} else {
		...
		*/
		...
		$body	=	'<table bgcolor="#FFFFFF" width="560" style="border:1px solid #ccc; opacity:0.8; font-family:Arial; font-size:14px; line-height:18px;" cellspacing="0" cellpadding="5" border="0" align="center">
						<tbody>
							...
							<tr>
								<td align="center"><span style="color:#000; font-size:2.2em; text-align:center; margin:0px;">para su registron</span></td>
							</tr>
							<tr>
								<td style="color:#4da6e1; font-size: 25px; padding-bottom: 10px; border-bottom: 1px solid #000;">Querido '.$signupusername.',</td>
							</tr>
							<tr>
								<td valign="top" align="left">
									<p style="color:#000; font-size: 29px; font-weight: normal; margin:15px 0px;">Gracias por</p>
									<p style="color:#000; font-size: 29px; font-weight: normal; margin:15px 0px;">registrarse de forma gratuita</p>
									<p style="color:#50BBE7; font-size: 29px;font-weight: normal; margin:15px 0px;">Cuenta .</p>
								</td>
							</tr>
							<tr>
								<td valign="top" align="left">
...
if(isset($_REQUEST['changepassword']) and $_REQUEST['changepassword']!=""){
	...
	$userData		=	$db->getAllRecords('administrador','id,numero_empleado',' AND token="'.$token.'"');
	if(isset($userData[0]['id']) and $userData[0]['id']!=""){
		...
		*/
		...
		$update		=	$db->update('administrador',$data,array('id'=>$userData[0]['id']));
		...
		echo '<div class="alert alert-danger p-1 mt-1"><i class="fa fa-fw fa-exclamation-triangle"></i> Token invalido <strong>Please try again!</strong></div>|***|0|***|index.php';
		exit;
	}
}