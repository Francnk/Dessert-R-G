...
<head>
	...
	<!--librerias necesarias para select-->
	<link rel="stylesheet" href="../libreria/bootstrap-select.css" />
	<!--<script src="../libreria/jquery.min.js"></script>-->
	<script src="../libreria/bootstrap.bundle.min.js"></script>
	<script src="../libreria/bootstrap-select.min.js"></script>
	<!--librerias necesarias para select-->
...
<body>
	...
	<br>
	<div class="container">
		<h2 class="font-weight-bold text-center">CUATRIMESTRE</h2>
		<div class="row">
			<div class="table-responsive ">
				<a href="#addnew" class="btn btn text-white  " style="background:#5daa49" data-toggle="modal"> <i class="fas fa-user"></i> Nuevo Cuatrimestre</a>
				...
				<table class="table table-striped table-hover  table-bordered " style="margin-top:20px;" id="tabladi">
					<thead class="p-3 mb-2 bg " style="background:#5daa49">
						<tr>
							<!--pag-->
							<th class="text-xl-center  text-white ">PROGRAMA EDUCATIVO</th>
							<th class="text-xl-center  text-white ">CUATRIMESTRE</th>
							<th class="text-xl-center  text-white ">PERIODO</th>
							<th class="text-xl-center  text-white ">AÑO</th>
					...
					<tbody>
						...
						try {
							...
							$sql = 'CALL mostrar_cuatrimestre_programa';
							/*$sql = 'SELECT * FROM cuatri_grupo';*/
							foreach ($db->query($sql) as $row) {
						?>
								<tr>

									<td><?php echo $row['programa_educativo']; ?></td>
									...
									<td>
										<center>
											<form method="POST" action="TablaGrupo/Grupo/Grupo.php">

												<input type="hidden" name="id_periodo" value="<?php echo $row['id_periodo']; ?>">
												<input type="hidden" name="id_programa" value="<?php echo $row['id_programa']; ?>">
												<input type="hidden" name="id_cuatrimestre" value="<?php echo $row['id_cuatrimestre']; ?>">
												<input type="hidden" name="id" value="<?php echo $row['id_cuatrimestre']; ?>">

												<button type="submit" name="agregar" class="btn1 btn-success  btn-sm" style="background:#5daa49">
													<span class="icon"><i class="fas fa-share-square"></i></span> grupo</button>
											</form>
											<br>

											<button href="#edit_<?php echo $row['id_cuatrimestre']; ?>" class="btn1 btn-success  btn-sm" style="background:#5daa49" data-toggle="modal"><span class="icon"><i class="fas fa-edit"></i></span> Editar </button>

											<br>
											<br>
											<button href="#delete_<?php echo $row['id_cuatrimestre']; ?>" class="btn1 btn-danger btn-sm" style="background:#be1e2d" data-toggle="modal"><span class="icon"><i class="fas fa-trash"></i></span> Borrar </button>
									...
									<?php include('../cuatrimestre/BorrarEditarModal.php'); ?>
								</tr>
